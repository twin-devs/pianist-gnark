...
type Proof struct {
	...
	Wpp fri.ProofOfProximity

	// Commitment to Z_x_i's for each i.
...
func Prove(spr *cs.SparseR1CS, pk *ProvingKey, fullWitness bn254witness.Witness, opt backend.ProverConfig) (*Proof, error) {
	...
	if err := commitToLRO(lCanonicalX, rCanonicalX, oCanonicalX, proof, pk.Vk.Iopp); err != nil {
		return nil, err
	}
	...
	if mpi.SelfRank == 0 {
		if proof.Wpp, err = pk.Vk.Iopp.BuildProofOfProximity(wCanonicalY); err != nil {
			return nil, err
		}
	}
	...
	if err := commitToQuotientX(hx1, hx2, hx3, hx4, proof, pk.Vk.Iopp); err != nil {
		return nil, err
	}
	...
	// TODO(dhruv): Generate opening proof of proof.Zpp
	// open Z at u*alpha
	...
	//TODO(dhruv): Replace the following with HxDigest of fri.
	...
	// TODO(dhruv): Replace the following with fri commitments.
...
func commitToLRO(bcl, bcr, bco []fr.Element, proof *Proof, iopp fri.Iopp) error {
	// Generate fri commitments for bcl, bcr and bco.
	// If selfRank == 0 then receive from others and return the slice of commitments.
	// else send commitment to master node and return nothing.
	var err error
	proof.LROpp[0], err = dfriCommit(bcl, iopp)
	if err != nil {
		return err
	}
	proof.LROpp[1], err = dfriCommit(bcr, iopp)
	if err != nil {
		return err
	}
	proof.LROpp[2], err = dfriCommit(bco, iopp)

	return err
}

// sendProofsOfProximity sends proofs of proximity to master node.
func sendProofsOfProximity(iopp fri.Iopp, pps ...fri.ProofOfProximity) error {
	for _, pp := range pps {
		subCommBytes, err := iopp.MarshalProofOfProximity(pp)
		if err != nil {
			return nil
		}

		// First send the length of bytes to expect
		err = mpi.SendBytes([]byte{byte(len(subCommBytes))}, 0)
		if err != nil {
			return err
		}

		// Now send the bytes for the commitment.
		err = mpi.SendBytes(subCommBytes, 0)
		if err != nil {
			return err
		}
	}
...
func commitToQuotientX(h1, h2, h3, h4 []fr.Element, proof *Proof, iopp fri.Iopp) error {
	var err error
	proof.Hxpp[0], err = dfriCommit(h1, iopp)
	if err != nil {
		return err
	}
	proof.Hxpp[1], err = dfriCommit(h2, iopp)
	if err != nil {
		return err
	}
	proof.Hxpp[2], err = dfriCommit(h3, iopp)
	if err != nil {
		return err
	}
	proof.Hxpp[3], err = dfriCommit(h4, iopp)
...
func dfriCommit(p []fr.Element, iopp fri.Iopp) ([]fri.ProofOfProximity, error) {
	var resp []fri.ProofOfProximity
	if mpi.SelfRank != 0 {
		comm, err := iopp.BuildProofOfProximity(p)
		if err != nil {
			return nil, err
		}

		err = sendProofsOfProximity(iopp, comm)
		if err != nil {
			return nil, err
		}

		return []fri.ProofOfProximity{}, nil
	}

	comm0, err := iopp.BuildProofOfProximity(p)
	if err != nil {
		return nil, err
	}

	resp = append(resp, comm0)

	// Receive commitments from slaves.
	for i := 1; i < int(mpi.WorldSize); i++ {
		subComLenO, err := mpi.ReceiveBytes(1, uint64(i))
		if err != nil {
			return nil, err
		}

		if len(subComLenO) != 1 {
			return nil, errors.New("invalid sub comm len")
		}

		subCommBytes, err := mpi.ReceiveBytes(uint64(subComLenO[0]), uint64(i))
		if err != nil {
			return nil, err
		}

		subCom, err := iopp.UnmarshalProofOfProximity(subCommBytes)
		if err != nil {
			return nil, err
		}

		resp = append(resp, subCom)
	}
