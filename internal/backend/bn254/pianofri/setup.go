...
func Setup(spr *cs.SparseR1CS, publicWitness bn254witness.Witness) (*ProvingKey, *VerifyingKey, error) {
	...
	// the domain will be rho*size_domain where size_domain is the next power
	// of 2 after pk.Domain[0].Cardinality, which is rho*pk.Domain[0].Cardinality
	tmpDomain := fft.NewDomain(rho * pk.Domain[0].Cardinality)
